...
class AdServerEnv(gym.Env):
    ...
    def seed(self, seed=None):
        self.np_random, seed = seeding.np_random(seed) #returns RandomState object and the seed
        return [seed]
    ...
    def step(self, action):
        ...
        reward = self.draw_click(action) #action is selecting an ad
        if reward == 1:
            clicks += 1
            ads[action].clicks += 1
    ...
    def render(self, mode='human', freeze=False, output_file=None): # pragma: no cover
        ...
        # Plot impressions
        plt.subplot2grid(grid_size, (2, 0), rowspan=3, colspan=2)
    ...
    def draw_click(self, action):
        ...
        if self.click_probabilities is None:
            ...
            #self.click_probabilities = np.array([], (self.num_ads,1), dtype=float)
            self.click_probabilities = np.zeros((self.num_ads, 1), dtype=float)
            for i in range(self.num_ads):
                self.click_probabilities[i] = self.np_random.uniform()*0.5
